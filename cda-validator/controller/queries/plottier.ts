...
let fakeRequest = {
    user: {
        usuario: ConfigPrivate.staticConfiguration.plottier.user,
        app: 'rup:prestacion:create',
        organizacion: 'sss'
    },
    ip: ConfigPrivate.staticConfiguration.plottier.ip,
    connection: {
        localAddress: ''
    }
};
...
WHERE   pac.numeroDocumento = '${dni}'`;
        ...
        await log(fakeRequest, 'microservices:integration:cda-validator', paciente.id, 'make:plottier:error', null, { documento: paciente.documento, query }, ex);
        throw ex;
    }
}
